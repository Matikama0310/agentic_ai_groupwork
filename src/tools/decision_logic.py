...
# Shared constants
# ---------------------------------------------------------------------------
MAX_INPUT_LENGTH = 50_000          # max chars for any text input
MAX_EMPLOYEES = 100_000
MAX_ANNUAL_REVENUE = 10_000_000_000  # $10B
MIN_PREMIUM = 50.00                # floor for any quote
MAX_PREMIUM = 50_000_000.00       # ceiling sanity check
...
def classify_naics_code(business_description: str, business_name: str = "") -> ToolResult:
    ...
    """
    try:
        if not isinstance(business_description, str):
            return ToolResult(False, {}, error="business_description must be a string")
        if not isinstance(business_name, str):
            business_name = ""

        business_description = business_description[:MAX_INPUT_LENGTH]
        business_name = business_name[:MAX_INPUT_LENGTH]
        ...
        logger.error(f"classify_naics_code failed: {e}")
        return ToolResult(False, {}, error=f"Classification failed: {e}")
...
def validate_against_guidelines(
    ...
    try:
        ...
        # 2. Check hard rules
        enriched_data = enriched_data if isinstance(enriched_data, dict) else {}
        loss_history = loss_history if isinstance(loss_history, dict) else {}

        values = {
            "credit_score": _safe_numeric(enriched_data.get("credit_score"), 0),
            "loss_ratio": _safe_numeric(loss_history.get("loss_ratio"), 0),
            "debt_to_equity": _safe_numeric(extracted_data.get("debt_to_equity"), 0),
            "years_in_business": _safe_numeric(extracted_data.get("years_in_business"), 0),
        }
        ...
        logger.error(f"validate_against_guidelines failed: {e}")
        return ToolResult(False, {}, error=f"Validation failed: {e}")
...
def calculate_risk_and_price(
    ...
    try:
        ...
        # Extract and clamp numeric values
        annual_revenue = _clamp(_safe_numeric(extracted_data.get("annual_revenue"), 100_000), 0, MAX_ANNUAL_REVENUE)
        employees = _clamp(_safe_numeric(extracted_data.get("employees"), 5), 0, MAX_EMPLOYEES)
        credit_score = _clamp(_safe_numeric(enriched_data.get("credit_score"), 650), 0, 850)
        crime_score = _clamp(
            _safe_numeric(enriched_data.get("property_risk", {}).get("crime_score") if isinstance(enriched_data.get("property_risk"), dict) else None, 50),
            0, 100,
        )
        total_losses = max(0, _safe_numeric(loss_history.get("total_losses"), 0))

        loss_ratio = (total_losses / annual_revenue) if annual_revenue > 0 else 0
        debt_to_equity = max(0, _safe_numeric(extracted_data.get("debt_to_equity"), 1.0))
        ...
        # Enforce premium bounds
        annual_premium = _clamp(annual_premium, MIN_PREMIUM, MAX_PREMIUM)
        ...
        logger.error(f"calculate_risk_and_price failed: {e}")
        return ToolResult(False, {}, error=f"Risk calculation failed: {e}")
